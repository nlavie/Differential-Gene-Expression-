...
for threshold in [0.05, 0.1]:
    for col in ['1-fdr h2c', '1-fdr c2h', '2-fdr h2c', '1-fdr h2hm', '1-fdr hm2h', '2-fdr h2hm', '1-fdr h2f', '1-fdr f2h', '2-fdr h2f']:
...
plot_OA(ttest, 'h2c 1-pvalue', 'Healthy to Convalescent DE (Healthy Over expressed)')
plot_OA(ttest, 'c2h 1-pvalue', 'Convalescent to Healthy DE (Healthy Under expressed)')
plot_OA(ttest, 'h2c 2-pvalue', 'Healthy to Convalescent DE')
...
healthy = [id for id, state in ids_to_state.items() if state == 'healthy control']
convalescent = [id for id, state in ids_to_state.items() if state == 'Convalescent']
...
expression_data = pd.read_csv('expression.csv').set_index('ID_REF')
...
def significant_genes(by_col, for_class):
    ...
    for gene in genes:
        ...
        healthy_expression = gene_expression.ix[healthy]
...
significant_genes('2-fdr h2c', 'Convalescent')
